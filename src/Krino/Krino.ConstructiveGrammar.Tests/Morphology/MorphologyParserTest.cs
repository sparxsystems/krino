...
namespace Krino.ConstructiveGrammar.Tests.Morphology
{
    ...
    public class MorphologyParserTest
    {
        ...
        public void FindFreeMorphemes_Similar()
        {
            ...
            var dictionary = new MorphologyParser(morphemes);

            var result = dictionary.Parse("writ", 1).ToList();
            ...
            result = dictionary.Parse("write", 1).ToList();
            Assert.AreEqual(1, result.Count);
            Assert.AreEqual("write", result[0].Value);
        }
        ...
        public void FindMorphemeSequences()
        {
            ...
            // prefix 're'
            var result = dictionary.Parse("rewrite", 0).ToList();
            Assert.AreEqual(1, result.Count);
            ...
            // suffix 'er'
            result = dictionary.Parse("reader", 0).ToList();
            Assert.AreEqual(1, result.Count);
            ...
            result = dictionary.Parse("extrarereaderless", 0).ToList();
            Assert.AreEqual(1, result.Count);
            ...
            result = dictionary.Parse("rebla", 0).ToList();
        ...
        public void FindMorphemeSequences_SuffixChangesFreeMorpheme()
        {
            ...
            var morphemeSequences = dictionary.Parse("writer", 1).ToList();
