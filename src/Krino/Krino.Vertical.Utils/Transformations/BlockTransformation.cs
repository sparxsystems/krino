...
namespace Krino.Vertical.Utils.Transformations
{
    ...
    public class BlockTransformation<T> : ITransformation<T>
    {
        private ITransformation<T>[] myTransformations;

        public BlockTransformation(params ITransformation<T>[] transformations)
        {
        ...
        public bool Equals(ITransformation<T> other) =>
            other is BlockTransformation<T> otherTransformation &&
